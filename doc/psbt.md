# PSBT Howto for Fokawa Core

Since Fokawa Core 0.17, an RPC interface exists for Partially Signed Fokawa
...
PSBT is an interchange format for Fokawa transactions that are not fully signed
...
Overall, the construction of a fully signed Fokawa transaction goes through the
following steps:
...
- An **Extractor** produces a valid Fokawa transaction (in network format)
...
#### Multisig with multiple Fokawa Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Fokawa Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `fokawa-cli` is used.
