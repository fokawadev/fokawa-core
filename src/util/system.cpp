...
const char * const BITCOIN_CONF_FILENAME = "fokawa.conf";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Fokawa";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Fokawa";
#else
    // Unix
    return pathRet / ".fokawa";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Bitcoin Core") == std::string::npos) {
        ...
        strYear.replace(strYear.find("2023"), sizeof("2023")-1, "2011-2023");
